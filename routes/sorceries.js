...
app.get('/api/sorceries/get/:name', async (req,res) => {
    const result = await sqlQuery.sqlQuery(`SELECT * FROM sorceries WHERE name = '${req.params.name.replaceAll('+', ' ').replaceAll("'", "\\'")}'`)
    res.json(result[0])
})
