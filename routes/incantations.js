...
app.get('/api/incantations/get/:name', async (req,res) => {
    const result = await sqlQuery.sqlQuery(`SELECT * FROM incantations WHERE name = '${req.params.name.replaceAll('+', ' ').replaceAll("'", "\\'")}'`)
    res.json(result[0])
})
